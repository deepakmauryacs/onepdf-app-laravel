...
<style>
  ...
  *{font-family:"DM Sans",system-ui,-apple-system,Segoe UI,Roboto,Arial,sans-serif}
  ...
  .pagination .page-link .bi{margin:0;font-size:16px}
</style>
@endpush
...
<div class="container py-3">
    ...
    <div class="card-header">
      <div class="files-toolbar">
        <div class="files-left">
          <div class="folder"><i class="bi bi-gem"></i> User Plans <span class="count">{{ $userPlans->total() }}</span></div>
          <form id="searchForm" class="search-wrap">
            <i class="bi bi-search"></i>
            <input id="searchInput" name="search" value="{{ $search }}" type="text" class="form-control search-input" placeholder="Search...">
          </form>
        </div>
      </div>
    </div>
  ...
  <nav class="mt-4 pagination-wrap" aria-label="User plans pagination">
    @if($userPlans->total())
      <div class="pager-summary">Showing {{ $userPlans->firstItem() }}–{{ $userPlans->lastItem() }} of {{ $userPlans->total() }}</div>
    @endif
