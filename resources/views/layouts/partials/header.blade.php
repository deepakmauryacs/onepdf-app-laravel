    ...
    <div id="nav" class="collapse navbar-collapse">
      <ul class="navbar-nav ms-auto mb-2 mb-lg-0 align-items-lg-center">
        ...
        <li class="nav-item ms-lg-3">
          <a class="btn btn-ghost btn-sm {{ request()->routeIs('login') ? 'active' : '' }}" href="{{ route('login') }}">Log in</a>
        </li>
        <li class="nav-item ms-2">
          <a class="btn btn-brand btn-sm {{ request()->routeIs('register') ? 'active' : '' }}" href="{{ route('register') }}">Start free</a>
        </li>
      </ul>
    </div>
  </div>
